...
int sunxi_chang_freq_safe(struct kbase_device *kbdev, unsigned long *freq, unsigned long u_volt)
{
	...
	if (!pll_gpu_name && !IS_ERR_OR_NULL(sunxi_data->pll_gpu))
	 ...
	 * once more.
	 */
	if (pll_gpu_name && strcmp(pll_gpu_name, gpu_parent_name) == 0)
	...
	if (kbdev->devfreq) {
		...
		kbdev->current_freqs[0] = sunxi_data->current_freq;
	}
...
int sunxi_platform_init(struct kbase_device *kbdev)
{
	...
	int clk_idx = 0;
#if IS_ENABLED(CONFIG_ARCH_SUN65IW1) || IS_ENABLED(CONFIG_REGULATOR)
	unsigned int i = 0;
#endif
	...
	sunxi_data->extra_gpu_clk[i] = devm_clk_get(kbdev->dev, "gpu-ahb-gate");
	if (IS_ERR_OR_NULL(sunxi_data->extra_gpu_clk[i])) {
		sunxi_info(kbdev->dev, "try to get gpu-ahb-gate clk fail\n");
		return -1;
	}
	i++;
	sunxi_data->extra_gpu_clk[i] = devm_clk_get(kbdev->dev, "gpu-axi-gate");
	if (IS_ERR_OR_NULL(sunxi_data->extra_gpu_clk[i])) {
		sunxi_info(kbdev->dev, "try to get gpu-axi-gate clk fail\n");
		return -1;
	}

	for (i = 0; i < BASE_MAX_NR_EXTRA_CLOCKS; i++) {
		if (sunxi_data->extra_gpu_clk[i]) {
			if (clk_prepare_enable(sunxi_data->extra_gpu_clk[i])) {
				sunxi_info(kbdev->dev, "enable gpu gate clk fail\n");
				return -1;
			}
		}
	}
	...
	for (clk_idx = 0; clk_idx < SUNXI_MAX_CLOCKS; clk_idx++) {
		sunxi_data->sunxi_clocks[clk_idx] = devm_clk_get(kbdev->dev,
				SUNXI_CLOCK_NAMES[clk_idx]);
		if (IS_ERR_OR_NULL(sunxi_data->sunxi_clocks[clk_idx])) {
			sunxi_info(kbdev->dev, "sunxi init gpu Failed to get %s\n",
					SUNXI_CLOCK_NAMES[clk_idx]);
			continue;
		}
		if (clk_prepare_enable(sunxi_data->sunxi_clocks[clk_idx])) {
			sunxi_err(kbdev->dev, "init failed to prepare and enable %s\n",
					SUNXI_CLOCK_NAMES[clk_idx]);
			return -1;
		}
	}
	...
	kbdev->independent_power = sunxi_data->independent_power;
...
#if IS_ENABLED(CONFIG_ARCH_SUN65IW1)
	sunxi_data->reset[1] = devm_reset_control_get(kbdev->dev, "rst_bus_gpu_axi");
	if (IS_ERR_OR_NULL(sunxi_data->reset[1])) {
		sunxi_info(kbdev->dev, "sunxi init gpu Failed to get bus_gpu_axi reset ctrl\n");
	}
#endif
	sunxi_data->max_freq = freq;
 ...
 */
	...
	if (!sunxi_data->is_perclk_set) {
		for (i = 0; i < BASE_MAX_NR_EXTRA_CLOCKS; i++) {
			if (sunxi_data->extra_gpu_clk[i]) {
				if (clk_prepare_enable(sunxi_data->extra_gpu_clk[i])) {
					sunxi_info(kbdev->dev, "power on enable gpu gate clk fail\n");
				}
			}
		}
	}
